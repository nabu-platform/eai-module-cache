		...
		for (CacheArtifact artifact : repository.getArtifacts(CacheArtifact.class)) {
			...
			try {
				if (artifact.getConfiguration().getCacheProvider() == null) {
					...
					ResourceCache cache = new ServiceAwareResourceCache(artifact.getConfiguration().getService(), artifact.getConfiguration().getCacheProvider().getCacheContainer(), 
						// defaults to 10 mb
						...
						// defaults to an hour
						artifact.getConfiguration().getAccessTimeout() == null ? 1000*60*60 : artifact.getConfiguration().getAccessTimeout(),
